...
//         printf("velocities %f %f %f\n",box->vx[i],box->vy[i],box->vz[i]);
    ...
    for(i=0;i<p->nAto;i++)
    {
        box->KE += 0.5*box->mass[i]*(box->vx[i]*box->vx[i] + box->vy[i]*box->vy[i] + box->vz[i]*box->vz[i]);
    }
...
void updateTrial(struct SYSTEM *box, struct PARAINPUT *p,int chainflag)
{
	...
	for(i=0;i<p->nAto;i++)
	{
		...
		{
			box->fRx[i] = sqrt(2.0*p->thermostatFact*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgx[i];
			box->fRy[i] = sqrt(2.0*p->thermostatFact*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgy[i];
			box->fRz[i] = sqrt(2.0*p->thermostatFact*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgz[i];
		}
		else // chromosome random forces
		{
			box->fRx[i] = sqrt(2.0*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgx[i];
			box->fRy[i] = sqrt(2.0*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgy[i];
			box->fRz[i] = sqrt(2.0*p->tem/(p->dt*p->gamma*box->radius[i]))*box->rgz[i];
		}
...
//         printf("%f old %f\n",box->actx[i],box->oldactx[i]);
		...
		box->trialX[i] = box->oldX[i] + p->dt*box->invmass[i]*((box->fx[i] + box->activity[i]*box->actx[i])/(p->gamma*box->radius[i])  + box->fRx[i]);
		box->trialY[i] = box->oldY[i] + p->dt*box->invmass[i]*((box->fy[i] + box->activity[i]*box->acty[i])/(p->gamma*box->radius[i])  + box->fRy[i]);
		box->trialZ[i] = box->oldZ[i] + p->dt*box->invmass[i]*((box->fz[i] + box->activity[i]*box->actz[i])/(p->gamma*box->radius[i])  + box->fRz[i]);
...
// 		box->ori[i] = box->ori[i] + 0.5*p->dt*box->vori[i];
    ...
    for(ii=0;ii<p->nChrom;ii++)
    {
        ...
        spamwx = box->torquex[i]/p->rotgamma; // plus some noise term for the Brownian rotation case
        spamwy = box->torquey[i]/p->rotgamma; // 
        spamwz = box->torquez[i]/p->rotgamma;
...
// 		printf("old q post qdot %f %f %f %f\n",box->oldqx[i],box->oldqy[i],box->oldqz[i],box->oldqw[i]);
        ...
        box->trialqx[i] = box->oldqx[i] + p->dt*box->invmass[i]*qdot[0];
        box->trialqy[i] = box->oldqy[i] + p->dt*box->invmass[i]*qdot[1];
        box->trialqz[i] = box->oldqz[i] + p->dt*box->invmass[i]*qdot[2];
        box->trialqw[i] = box->oldqw[i] + p->dt*box->invmass[i]*qdot[3];
...
// 			printf("sep %d %f %f -- restoring?? %f\n",j,rij,r0,box->fx[j]*diffX + box->fy[j]*diffY + box->fz[j]*diffZ);
			
            if(rij >= r0)
            {
//                 if(box->probrij/box->probr0 < rij/r0)
//                 {
//                     box->probrij=rij;
//                     box->probr0=r0;
//                     box->probi = i;
//                 }
//                 box->printflag[i] = box->type[i]+2;
//                 box->printflag[n1] = box->type[n1] + 2;
                chainflag=1; // extension beyond limits
                
//                 printf("%d -- forces centre %f %f %f + %f %f %f -- forces bead %f %f %f\n",j,box->fx[i],box->fy[i],box->fz[i],box->fRx[i],box->fRy[i],box->fRz[i],box->fx[j],box->fy[j],box->fz[j]);
// 				exit(0);
            }
...
// 	for(i=0;i<p->nAto;i++)
// 	{
// 		if( sqrt(p->Rw*p->Rw) - sqrt(box->trialX[i]*box->trialX[i] + box->trialY[i]*box->trialY[i] + box->trialZ[i]*box->trialZ[i]) <= 0.25*p->sigma[2*box->type[i]])
// 		{
// 			printf("wall dist for particle %d type %d coming as %f -- typical %f\n",i,box->type[i],p->Rw*p->Rw - (box->trialX[i]*box->trialX[i] + box->trialY[i]*box->trialY[i] + box->trialZ[i]*box->trialZ[i]),0.5*p->sigma[2*box->type[i]]);
// 			chainflag=1;
// 		}
// 			
// 	}
...
//         printf("surf forces %d - %f %f %f -- random %f %f %f\n",i,box->surffx[i],box->surffy[i],box->surffz[i],sqrt(2.0*p->surfDiffu/p->dt)*rgx,sqrt(2.0*p->surfDiffu/p->dt)*rgy,sqrt(2.0*p->surfDiffu/p->dt)*rgz);
        ...
        box->surfPointx[i] = box->surfPointx[i] + p->dt*(box->surffx[i] + sqrt(2.0*p->surfDiffu/p->dt)*rgx);
        box->surfPointy[i] = box->surfPointy[i] + p->dt*(box->surffy[i] + sqrt(2.0*p->surfDiffu/p->dt)*rgy);
        box->surfPointz[i] = box->surfPointz[i] + p->dt*(box->surffz[i] + sqrt(2.0*p->surfDiffu/p->dt)*rgz);
	}
...
//     vely = p->sig0y/p->surfEta + (p->sigdel/p->surfEta)*rgy - (p->surfSpringSelf/p->surfEta)*p->ellb;
    ...
    velx = currsig0x/p->surfEta - (p->surfSpringSelf/p->surfEta)*p->ella;
...
//     velz = p->sig0z/p->surfEta + p->sigdel/p->surfEta*rgz - (p->surfSpringSelf/p->surfEta)*p->ellc;
    velz = currsig0z/p->surfEta - (p->surfSpringSelf/p->surfEta)*p->ellc;
