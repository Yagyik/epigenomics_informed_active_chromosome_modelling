void readPara(FILE *fp, struct PARAINPUT *p, struct SYSTEM *box)
{
	...
	// Initialize run-time values at the start of the schedule.
	p->surfEta = 1.0/(p->surfDiffu);
	...
	// Use the non-volume-preserving branch used by production scripts.
	...
	p->dea=(p->ellaf - p->ellai)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->deb=(p->ellbf - p->ellbi)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->dec=(p->ellcf - p->ellci)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));

	// Precompute linear schedule increments consumed by the integrator.
	p->dtem = (p->temf - p->temi)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->dssc = (p->surfSpringCrossf - p->surfSpringCrossi)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->dsigdel = (p->sigdelf - p->sigdeli)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->dls = (p->lamin_scalef - p->lamin_scalei)*p->dewin/(1.0*(0.75*p->totRun - p->eqRun));
	p->dsigactdel = (p->sigactdelf - p->sigactdeli)/(1.0*(0.75*p->totRun - p->eqRun));
	p->dactscale = (p->actscalef - p->actscalei)/(1.0*(0.75*p->totRun - p->eqRun));
...
void readTopo(FILE *fp, struct PARAINPUT *p, struct SYSTEM *box)
{
	...
	int chromindex, type;
	double spamx,spamy,spamz,orix,oriy,oriz,rad,mass,k_spring,act,lamin;
...
{
	// Read sparse pair interactions by global patch indices.
	...
	for(ii=0;ii<p->nChrom;ii++)
	{
		for(jj=1;jj<=box->patchList[ii][0];jj++)
		{
			...
			// Initialize patch tags: default non-lamin vs lamin.
	...
	while(!feof(fp))
	{
		...
		// Mark each interacting patch with the target chromosome type for downstream force logic.
...
// Legacy reader for the older (chrom,patch,chrom,patch,eps) matrix format.
...
//              printf("printing chrom info no issue %d -- %d\n",ii,structbox.patchList[ii][0]);
        for(jj=1;jj<=box->patchList[ii][0];jj++)
        {
			...
			box->k_spring[j]/p->spring_scale,box->activity[j]/p->actscale,box->lamin[j]/p->lamin_scale);
...
void readExpectedMat(struct PARAINPUT *p, struct SYSTEM *box)
{
	...
	// The first token is the step/index marker from constraints dumps.
