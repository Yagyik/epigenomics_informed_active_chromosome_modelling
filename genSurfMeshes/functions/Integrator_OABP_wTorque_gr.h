double gauss_var_AT(double mu,double sig,struct PARAINPUT *p)
{
...
int init_vel(struct PARAINPUT *p,struct SYSTEM *box)
{
    ...
    for(i=0;i<p->nAto;i++)
    {
        box->vx[i]=gauss_var_AT(0,sig,&(*p)); ///box->mass[i]; // init momentum and get vel
        box->vy[i]=gauss_var_AT(0,sig,&(*p)); ///box->mass[i];
        box->vz[i]=gauss_var_AT(0,sig,&(*p)); ///box->mass[i];
...
void integrateLangevinEM(struct SYSTEM *box,struct PARAINPUT *p)
{
	...
	for(i=0;i<p->nAto;i++)
	{
		...
		box->rgx[i] = gauss_var_AT(0,1,&(*p));
		box->rgy[i] = gauss_var_AT(0,1,&(*p));
		box->rgz[i] = gauss_var_AT(0,1,&(*p));

        // rotational noise for active forces

        box->argx[i] = gauss_var_AT(0,1,&(*p));
		box->argy[i] = gauss_var_AT(0,1,&(*p));
		box->argz[i] = gauss_var_AT(0,1,&(*p));
...
//     {
//         box->surfrgx = gauss_var_AT(0,1,&(*p));
//         box->surfrgy = gauss_var_AT(0,1,&(*p));
//         box->surfrgz = gauss_var_AT(0,1,&(*p));
//     }
//
