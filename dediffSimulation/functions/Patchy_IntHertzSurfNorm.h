...
// 					wth = 0.4*p->epsij_scale/p->eps_blob;
					wth = 0.2; //*p->epsij_scale/p->eps_blob;
...
// 					ath = 1.0 + 0.4*p->epsij_scale/p->eps_blob;
					ath = 1.0 + 0.3; //*p->epsij_scale/p->eps_blob;
...
// 				patchcut = 1.0*(0.5*avesep + box->radius[i]);
			...
			/// get attractive interactions out of the way

			if(box->lamin[i] > 0.0)
			{
				if(rijsq <= 4.0*patchcut*patchcut)
				{
					ath = 1.0 + 0.1*(p->lamin_scale/p->eps_blob); // lamin_scale = box->lamin[i] for all i always
// 					wth = 0.15*(p->lamin_scale/box->lamin[i]);
					wth = 0.1; //*(p->lamin_scale/box->lamin[i]);

					invwidthwell = 1.0/(wth*patchcut);
					tanhfact = tanh((rij - ath*patchcut)*invwidthwell);

					ff = -box->lamin[i]*p->eps_blob*(1.0 - tanhfact*tanhfact)*invwidthwell;

					en += 0.5*box->lamin[i]*p->eps_blob*tanhfact - 0.5*box->lamin[i]*p->eps_blob;
// 						box->IMEne += 0.5*box->lamin[k]*tanhfact - 0.5*box->lamin[k];
					...
					if(rijsq <= sqr(pow(2.0,1.0/6.0)*patchcut))
					{
					lamintspam = 0.5 - 0.5*tanh((rijanch-1.2*patchcut)/(0.1*1.2*patchcut));

					box->isIntermingling[i] = MAX(box->isIntermingling[i],lamintspam);

					box->isMembIntermingling[j] = 0.5 - 0.5*tanh((rijanch-1.2*patchcut)/(0.1*1.2*patchcut));
					if(rij < minrijlam)
						minrijlam = rij;
					}
				}
