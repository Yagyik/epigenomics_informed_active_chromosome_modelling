...
int main (int argc, char *argv[])
{
	...
	char filepart[200],filename[500],thermofilename[500],sffilename[500];
	
	int i,t=0,spampairs=0;
	int mdstart;
	double rgx,rgtheta,rgz;
	...
	// Read the global constraints vector used for IPD output tracking.
	structbox.fpIPD=fopen("ConstraintsVector.dat","r");
	if(structbox.fpIPD==NULL)
	{
		printf("Error: ConstraintsVector.dat not found!\n");
		exit(1);
	}
	...
	// Fresh start branch: read topology, interactions, and pre-prepared surface files.
	if(p.initRead==0)
	{
	...
	// Restart branch: recover RNG/config state and continue from checkpointed files.
	if(p.initRead ==1)
	{
	...
	// Seed energies before entering equilibration / production loops.
	...
	int annealcount=0;
	structbox.activeAccum = 0;
	structbox.osmoflag=0;


	// save olds and replace with new
	double oldtem = p.tem;
	double oldsigactdel = p.sigactdel;
	// For fresh starts, do a short passive relaxation before restoring activity.
	if(p.initRead == 0)
	{
		p.sigactdel = 0.001;
		p.tem = 0.0001;
		for(i=0;i<p.nAto;i++)
		{
			structbox.oldactivity[i] = structbox.activity[i];
			structbox.activity[i] = 0.0;
		}
		...
		for(i=0;i<p.nAto;i++)
		{
			structbox.activity[i] = structbox.oldactivity[i];
		}
	}
	p.tem = oldtem;
	p.sigactdel = oldsigactdel;

	// Main production loop: diagnostics, dumps, restart snapshots, then one integration step.
	for(t=mdstart;t<p.totRun;t++)
    {
		if(t%p.thermo==0 || t==0)
		{
			printf("Step %d -- IMe %f PE %f chPE %f sfPE %f KE %f geom %f (%f) %f (%f) %f (%f) (angle) %f  \
			-- str %f %f restore %f %f act %f %f -- %f %f -- surfAct %d\n",t,structbox.IMEne/p.epsij_scale,structbox.potEne,structbox.chromPot, \
			...
			structbox.activeAccum);

			fprintf(fpThermo,"%d %f %f %f %f %f %f %f %f %f %d\n",t,structbox.IMEne/p.epsij_scale, \
			structbox.potEne,structbox.KE,p.ella,p.ellb,p.ellc,structbox.currsig0x,structbox.currsig0z,structbox.sigtheta,structbox.activeAccum);
		...
		// During anneal window, update control parameters and geometry targets.
        if(t>=p.eqRun && t < 0.5*p.totRun && t%p.dewin==0)
		{
			annealcount+=1;
			...
			p.sigactdel = p.sigactdelf;
			p.actscale = p.actscalef;
			...
			{
				p.deb = (p.ellbf - p.ellb)*p.dewin/(1.0*(0.5*p.totRun - t));
				printf("current eb %f -- will change to %f -- %f %f\n",p.ellb,p.ellbi + p.deb*annealcount,p.deb,0.5*p.totRun-t);
				p.ellb = p.ellbi + p.deb;
				updateGeom(&p,&structbox,structbox.currsig0x,structbox.currsig0z);

				printf("annealing geometry e %f sig %f -- e %f sig %f -- e %f sig %f -- tem %f actsc %f sgacdel %f -- lmn %f\n",p.ella,p.sig0x,p.ellb,p.sig0y,p.ellc,p.sig0z,p.tem,p.actscale,p.sigactdel,p.lamin_scale);
			}
		}
