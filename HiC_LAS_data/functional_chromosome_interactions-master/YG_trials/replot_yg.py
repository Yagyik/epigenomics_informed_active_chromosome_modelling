...
def map_pos2rownum(df, row_pos):
        if np.where(df.index.values == row_pos)[0].size > 0:
                return np.where(df.index.values == row_pos)[0][0]
        else:
                return -1
def map_pos2colnum(df, row_pos):
##        print('\n',row_pos,int(row_pos),str(int(row_pos)))
##        print(df.columns.values)
##        print(np.where(df.columns.values == str(int(row_pos))),np.where(df.columns.values == str(int(row_pos)))[0])
        spam = np.where(df.columns.values == str(int(row_pos)))
##        print(spam,spam[0],spam[0].size)
        spam2 = np.where(df.columns.values == row_pos)
##        print(spam2,spam2[0],spam2[0].size)
        if spam[0].size > 0:
                return spam[0][0]
        else:
                return -1
...
def numclust_avg(pair):
        ...
        if (os.path.isfile(fname) == True):
                ...
                plt.figure()
                plt.plot(range(df_intermingling.shape[0]), df_intermingling['score'], 'o-')
                plt.xlabel('Cluster #')
                plt.ylabel('Score')
                ...
                plt.figure()
                plt.plot(range(df_intermingling.shape[0]), df_intermingling['avg'], 'o-')
                plt.xlabel('Cluster #')
                plt.ylabel('Average')
...
def determine_min_max_hic():
    ...
    for pair in chr_pairs:
        chr1, chr2 = pair
        fname = config["HIC_NEW_DIR"]  + 'intermingling_regions.chr' + str(chr1) + '_chr' + str(chr2) + '.binned.csv'
        if (os.path.isfile(fname) == True):
                # read in hic matrix
...
##              data = df.as_matrix()
                ...
                for num in range(0, len(df_intermingling)):
                        ...
                        print(chr1,chr2,start_row,stop_row,start_col,stop_col,(stop_row - start_row)*(stop_col - start_col),totCon,spamMaxCon,maxCon)

                        if stop_row - start_row > 0 and stop_col - start_col >0:
                                totCon += np.sqrt((stop_row - start_row)*(stop_col - start_col))

                        if start_row != old_stop_row and start_col != old_stop_col: # end of a region
                                # compare the final region to old max
                                if spamMaxCon > maxCon:
                                        maxCon = spamMaxCon
                                        spamMaxCon=0 # reset
                        else:
                                if (stop_row - start_row)*(stop_col - start_col) > 0:
                                        spamMaxCon += np.sqrt((stop_row - start_row)*(stop_col - start_col))
                        

                        
                        old_stop_row=stop_row
                        old_stop_col=stop_col

                        # draw vertical lines - columns are same
                        if start_row >=0 and start_col >=0 and stop_row >=0 and stop_col >=0:
...
##      Parallel(n_jobs = config['NUM_PROC'])(delayed(numclust_avg)(pair) for pair in chr_pairs)
        ...
        ## YG addition to get contact matrix
        res = Parallel(n_jobs = config['NUM_PROC'])(delayed(draw_identified_LASregions)(pair, minl, maxl) for pair in chr_pairs)
        maxCont = [item[0] for item in res]
        totCont = [item[1] for item in res]
        print(maxCont)
        print(totCont)

        # above should be a couple of 1D lists corresponding to each pair in chr_pairs
        x = np.arange(1,23)
        y = np.arange(1,23)
        maxContArr=np.zeros([22,22],dtype=float)
        totContArr=np.zeros([22,22],dtype=float)
        for i in range(len(chr_pairs)):
                ch1 = chr_pairs[i][0]
                ch2 = chr_pairs[i][1]
                if ch1 == 'X' or ch2 == 'X':
                        continue
                else:
                        maxContArr[ch1-1,ch2-1] = maxCont[i]
                        totContArr[ch1-1,ch2-1] = totCont[i]
                        maxContArr[ch2-1,ch1-1] = maxCont[i]
                        totContArr[ch2-1,ch1-1] = totCont[i]


        # array populated -- now make meshgrid and plot it
##        print(maxContArr,totContArr)
        fig = plt.imshow(totContArr,cmap='Blues',extent=[1,22,1,22],origin='upper')
        fig.axes.set_autoscale_on(False)
##        plt.scatter(x.tolist(),y.tolist(),400,facecolors='none')
        cbar = plt.colorbar()
        cbar.set_label('Hi-C contact size')
        plt.xticks(x.tolist())
        plt.yticks(y.tolist(),y.tolist()[::-1])
        plt.xlabel('Chromosome #')
        plt.ylabel('Chromosome #')
        plt.savefig('trial_tot.png')
        plt.close()
...
##        fig = plt.imshow(maxContArr,cmap='YlOrRd', extent=[1,23,1,23],origin='upper')
        fig = plt.imshow(maxContArr,plt.cm.get_cmap('YlOrRd'),extent=[1,22,1,22],origin='upper')
        fig.axes.set_autoscale_on(False)
        cbar = plt.colorbar()
        cbar.set_label('Hi-C largest contact')
##        cmap = mpl.cm.YlOrRd
##        norm=mpl.colors.Normalize(vmin=min(maxCont),vmax=max(maxCont))
##        plt.scatter(x.tolist(),y.tolist(),400,facecolors='none')
##        fig.colorbar(mpl.cm.ScalarMappable(norm=norm,cmap=cmap),orientation='vertical',label='Hi-C largest contact')
##        cbar.set_label('Hi-C largest contact')
        
        plt.xlabel('Chromosome #')
        plt.ylabel('Chromosome #')
        plt.savefig('trial_max.png')
        plt.close()

        ## print out our trial_max matrix
        
        f=open("trial_max.dat","w")
        for i in range(len(chr_pairs)):
                ch1 = chr_pairs[i][0]
                ch2 = chr_pairs[i][1]
                if ch1 == 'X' or ch2 == 'X':
                        ...
                        f.write("%d %d %f %f\n" %(ch1,ch2,maxContArr[ch1-1,ch2-1],totContArr[ch1-1,ch2-1]))
        f.close()
