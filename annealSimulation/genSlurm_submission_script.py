...
def generate_script_text(
    ...
    job_name = str(params.get("job_name", "cm_anneal"))
    partition = str(params.get("partition", "daily"))
    nodes = int(params.get("nodes", 4))
    ntasks = int(params.get("ntasks", 176))
    ...
    lines.extend(["", "wait", 'echo "All anneal commands completed."', ""])
    return "\n".join(lines)
...
def main() -> None:
    ...
    parser.add_argument("--stage", default="anneal_simulation")
    ...
    anneal_dir = project_root / "annealSimulation"
    ...
    run_script = str(params.get("run_script", "run_46ch_script_usePrepped_reprog.sh"))
    output_script = (
        Path(args.output_script)
        if args.output_script
        else anneal_dir / str(params.get("submit_script_name", "submit_anneal_generated.sbatch"))
    )
    ...
    if do_submit:
        ...
        print("[cmd]", " ".join(cmd))
        if not args.dry_run:
            subprocess.run(cmd, cwd=str(anneal_dir), check=True)
